...
print("Bienvenido/a al registro de actividades diarias")
...
while True:
    ...
    if opcion == "1":
        cantidad_act = int(input("¿Cuántas actividades desea ingresar? (minimo 3): "))

        while cantidad_act < 3:

           cantidad_act = int(input("Cantidad de actividad no valida, por favor ingrese nuevamente (Minimo 3): "))

        while contador < cantidad_act:

            nombre_act = input("Ingrese el nombre de la actividad realizada, por favor: ")
            tiempo_act = int(input("Ingrese el tiempo (min) que se demoró para realizar su actividad, por favor: "))
            suma_tiempo += tiempo_act
            contador +=1

        
    if opcion == "2":
        print(f"Cantidad de tiempo total registrado {suma_tiempo} minutos")

        if suma_tiempo > 180:
           print("El tiempo a diario se excede...")

        else:
            print("Tiempo a diario adecuado.")
    ...
    print("Opción inválida, por favor ingrese una opción que aparezca en el menú: ")
