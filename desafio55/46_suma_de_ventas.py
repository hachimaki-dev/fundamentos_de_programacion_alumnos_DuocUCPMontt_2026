...
# Restricciones: No uses la función rápida `sum()`. Extrae solamente los números usando la herramienta `.values()` y 
# súmalos uno por uno adentro de un ciclo `for`. Imprime el total.

ventas = {"LocalA": 150, "LocalB": 300, "LocalC": 100}
total_ventas = 0

for venta_local in ventas.values():
    total_ventas += venta_local
