...
while True:
    ...
    if seleccion == seleccion1 :
        actividades = int(input("¿Cuantas actividades solicitas regitrar?: "))
        if actividades >= 3:
            for i in range(actividades):
                nombre = input("¿Cual es el nombre de la actividad?: ")
                tiempo = int(input("¿Cuantos minutos te toma dicha actividad?: "))
                acumulador += tiempo
                
        else:
            actividades < 3
            print(" ")
            print("Este valor debe ser un número entero mayor o igual a 3")
            print(" ")

    elif seleccion == seleccion2 :
        print(f"El tiempo total acumulado en las actividades es de {acumulador}")
        if acumulador > 180 :
            print(" ")
