...
def iniciar_programa():
    while True:
        ...
        if opcion_de_usuario == "1":
            ...
            nombre_a_buscar = input("ingrese el nombre del bicho")
            indece_del_bicho =buscar_bicho_por_nombre(nombre_a_buscar)
            if indece_del_bicho is not None:
                print("bicho encontrado")
            else:
                print("no existe")
            print("LLAMAR A LA FUNCION QUE BUSCA")
        elif opcion_de_usuario == "3":
            nombre_a_liminar = input("ingrese el nombre del que desea eliminar")
            fue_eliminado = eliminar_bicho_por_nombre(nombre_a_liminar)
            

            print("LLAMAR A LA FUNCION QUE ELIMINA BICHO")
...
def agregar_bicho():
    ...
    if se_registro:
        ...
        return False
def buscar_bicho_por_nombre(nombre_del_a_buscar_por_parametro):
    for cada_bichos in lista_de_todos_los_bichos:
        if cada_bichos["nombre_especie"] == nombre_del_a_buscar_por_parametro:
            print("existe")
            indece_bicho_encontrado = lista_de_todos_los_bichos.index(cada_bichos)
            return indece_bicho_encontrado
...
def actualizar_si_es_peligroso():
    for cada_bichos in lista_de_todos_los_bichos:
        if cada_bichos["peligrosidad_especie"] >= 7.0:
            print("es peligroso")
            indece_bicho_encontrado = lista_de_todos_los_bichos.index(cada_bichos)
            return indece_bicho_encontrado
