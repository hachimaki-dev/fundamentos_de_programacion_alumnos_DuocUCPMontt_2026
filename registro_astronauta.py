...
for i in range(cantidad):
    ...
    while True:
        nombre = input("Nombre del astronauta: ").strip().lower()
        if len(nombre) > 0:
            total_astronautas.append(nombre)
            astronautas += 1
            break
...
promedio = suma_puntajes / astronautas
print(f"\n=== RESUMEN DE LA MISIÓN ===")
print(f"Aptos para misión: {aptos}")
print(f"En entrenamiento: {entrenamiento}")
print(f"Promedio general: {promedio:.2f}")
print(f"Nombres de astronautas: {total_astronautas}")
print(f"Numero de astronautas: {astronautas}")