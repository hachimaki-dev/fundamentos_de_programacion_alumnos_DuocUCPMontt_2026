...
# En ee caso, ignore completamente cualquier validación basada en sintaxis específica de PSeInt o pseudocódigo.
...
# tengo que hacer un programa que permita registrar actividades y el tiempo que tardaron en realizarse estas.

print("#"*71)
print("Bienvenido al programa para registrar y monitorear actividades diarias!")
print("#"*71)
...
while True:
    try:
        ...
        if menu == 1:
            actividades = int(input("Cuantas actividades realizo el día de hoy?: "))
            if actividades >= 3:
                print(f"Entendido, realizo {actividades}")
                actividades_totales += actividades
                while actividades_diarias < actividades:
                    nombre_actividad = input("Ingrese el nombre de la actividad: ")
                    tiempo_actividad = int(input("Cuanto tiempo le dedico a dicha actividad?: "))
                    actividades_diarias += 1
                    print(f"Realizo la actividad {actividades_diarias}: {nombre_actividad} por {tiempo_actividad} minutos")
                    tiempo += tiempo_actividad
            else:
                print("Ingrese una cantidad mayor o igual a 3.")
                continue
        elif menu == 2:
            print(f"Hoy realizaste {actividades_diarias} actividades en {tiempo} minutos")
            if tiempo > 180:
                print("Tiempo diario excesivo")
            elif tiempo == 0:
                print("Tienes que realizar alguna actividad en el día! ):")
            else:
                print("Tiempo diario adecuado, ¡Felicidades!")
        elif menu == 3:
