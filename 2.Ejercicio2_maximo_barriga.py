...
while continuar :
    ...
    if opcion == 1 :
        actividades_cantidad = int(input("--Cantidad de actividades a registrar : "))
        actividades_contador = 0
        
        while actividades_cantidad != actividades_contador :
            
            if actividades_cantidad >= 3 :
                print("--Ingrese nombre de la actividad--")
                nombre_actividad = input("--Nombre :")
                print("--¿Cuanto tiempo conlleva esta actividad?--")
                tiempo_actividad = int(input("--Ingrese tiempo (En minutos) : "))
                actividades_contador = actividades_contador + 1
                tiempo_total_acumulado += tiempo_actividad
            
            
            
            elif actividades_cantidad < 3 :
                print("--numero invalido--")
                print("--Fin del registro--")
                exit()
                #El exit lo habia anotado en mi hojita que no sirvio pa na mucho xd, lo vi en un video de tis tos de python
    
    elif opcion == 2 :
        print(f"--Su tiempo total acumulado de las actividades registradas es {tiempo_total_acumulado}--")
        if tiempo_total_acumulado > 180 :
            print("--Tiempo diario excesivo--")
        elif tiempo_total_acumulado <= 180 :
