...
while bandera:
    ...
    print("3. salir")
    opcion_ingresada=int(input("Porfavor ingrese una opcion valida"))

    if opcion_ingresada == 1:
        print("haz elegido la opcion {opcion_ingresada}")
        numero_actividades = int(input("¿cuantas actividades quieres realizar?"))
        if numero_actividades > 2:
            while bandera1:
                contador = contador + 1
                nombre_actividad = input(f"cual es tu actividad {contador}")
                tiempo = int(input(f"¿cuanto dura tu actividad {contador}(tiempo en minutos)"))
                acumulador = acumulador + tiempo
                if contador == numero_actividades:
                    print("se han registrado las{contador} actividades")
                    contador = 0
                    break


        else:
            print("tienes que escoger al menos 3 actividades")
            print("volver al menu")

    elif opcion_ingresada == 2:
        print(f"escogiste la opcion {opcion_ingresada}")
        print("cargando el tiempo")     
        print(f"su tiempo total es de{acumulador}minutos")
        
        if acumulador > 180:
            print("tiempo diario en exceso")
            print("volviendo a las opciones")
            
        else:    
            print("tiempo adecuado") 
            print("volviendo a las opciones validas")
    
    
    elif opcion_ingresada == 3:
        print("usted ha escogido {opciion_ingresasa}")
