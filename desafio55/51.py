...
#Restricciones: Usa `.split()` para picar la frase en una lista de palabras. 
#Usa un `for` para recorrerlas. Usa `if palabra in freq:` para evitar que el código explote con palabras que no te interesan (como 'servicio' o 'precio').
#Imprime tu diccionario contador al terminar. Pista de Ayuda:Usa if palabra in freq: para asegurar que sumas correctamente.
freq = {'buen': 0, 'mal': 0}
