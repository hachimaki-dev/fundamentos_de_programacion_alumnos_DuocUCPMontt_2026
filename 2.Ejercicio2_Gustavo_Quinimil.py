...
while menu :
    ...
    if eleccion_del_usuario == 1 :
        cantidad_de_actividades = int(input("Cuantas actividades desea registrar (Obligatorio: 3 o más): "))
        if cantidad_de_actividades >= 3 :
            contador = 0
            acumulador = 0
            while contador < cantidad_de_actividades :
                registro_de_actividades = str(input("Ingrese el nombre de la actividad: "))
                minuto_por_actividades = int(input("Ingrese cuantos minutos realiza en la acividad: "))
                contador += 1
                acumulador += minuto_por_actividades
        else :
            print("Cantidad invalida")
    
    elif eleccion_del_usuario == 2 :
        print(f"Tiempo total: {acumulador} minutos")
