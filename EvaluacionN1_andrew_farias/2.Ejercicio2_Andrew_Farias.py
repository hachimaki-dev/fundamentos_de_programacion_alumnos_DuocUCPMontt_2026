...
while menu == True:
    ...
    if opcion_usuario > 3 or opcion_usuario <= 0:
        print("Ingresa una opcion valida")

    if opcion_usuario == registrar_actividades:
        numero_actividades = int(input("Ingrese el número de actividades: "))
        if numero_actividades >= 3:
            for i in range(numero_actividades):
                nombre_actividades += input("Ingresa el nombre de la actividad: ")
                tiempo_actividades += int(input("Ingresa el tiempo en minutos: "))
                
        else:
            print("debes ingresar minimo 3 actividades")

    if opcion_usuario == analisis_tiempo:
        print(f"\n{tiempo_actividades} Minutos en {numero_actividades} Actividades: \n{nombre_actividades}")
        if tiempo_actividades > tiempo:
