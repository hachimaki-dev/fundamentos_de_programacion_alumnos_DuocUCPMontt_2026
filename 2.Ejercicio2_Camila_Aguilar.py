...
while True:
    print("Menu: \n1) Registrar actividades\n2) Mostrar análisis de tiempo\n3) Salir")
    opcion_elegida = int(input("Ingrese su opción elegida: "))
    if opcion_elegida == 1:
        cantidad_actividades = int(input("Ingrese la cantidad de actividades a registrar: "))
        numero_actividad = 1
        if cantidad_actividades >= 3:
            while cantidad_actividades > 0:
                nombre_actividad = input(f"Ingrese el nombre de la actividad {numero_actividad}: ")
                tiempo_actividad = int(input(f"Ingrese cantidad de minutos de la actividad {numero_actividad}: "))
                tiempo_total = tiempo_total + tiempo_actividad
                cantidad_actividades = cantidad_actividades - 1
                numero_actividad += 1
        else:
            print("La cantidad de actividades a registrar debe ser mayor o igual a 3")
    elif opcion_elegida == 2:
        print(f"Tiempo total registrado: {tiempo_total}")
        if tiempo_total > 180:
