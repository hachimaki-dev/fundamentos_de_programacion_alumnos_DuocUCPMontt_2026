...
def procesoChechIn():
  ...
  while True:
    ...
    try:
      ...
      pedir_hbitaciones_disponibles = int(input('Ingrese la cantidad de habitaciones que desea pedir: '))

      if pedir_hbitaciones_disponibles > 0:
        if habitaciones_disponibles >= pedir_hbitaciones_disponibles:
          habitaciones_disponibles -= pedir_hbitaciones_disponibles
        
          print("Se ha realizado el check-in de forma exitosa")
          break

    except ValueError:

      print('Error: Ingrese un numero natural valido')

  return pedir_hbitaciones_disponibles
