...
Restricciones: Recorre las cosas del piso con un `for`. Agrégalas a tu mochila. Justo después de agregar, revisa si el largo (`len()`) de tu mochila es igual a 4. 
Si es así, imprime 'Lleno' y corta el ciclo con `break`. Al final, imprime cómo quedó tu mochila. """
...
for recoger in piso :
    mochila.append(recoger)
    tamano_mochila = len(mochila)
    if tamano_mochila == 4:
        print("lleno")
        break
print(f"{mochila}")