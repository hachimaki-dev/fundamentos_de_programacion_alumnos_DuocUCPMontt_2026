...
print("Registro de activides diarias")
...
while True:
    ...
    #opcion 1
    if opcion_elegida == 1:
        actividad_ingresada = int(input("Ingrese la cantidad de actividades a registrar: "))
        cantidad_total_actividades = 0
        tiempo_total_actividades = 0

        if actividad_ingresada >= 3:
            for i in range(actividad_ingresada):

                input(f"\nIngrese el nombre de la actividad número {i+1}: ")
                tiempo_actividades = int(input("Ingrese el tiempo que duro esta actividad en minutos (minutos): "))

                cantidad_total_actividades += 1
                tiempo_total_actividades += tiempo_actividades
        
    #opcion 2
    elif opcion_elegida == 2:
        if tiempo_total_actividades > 180:
            print("\nSu tiempo diario excesivo\n")
        else:
            print("\nSu tiempo diario adecuado\n")
