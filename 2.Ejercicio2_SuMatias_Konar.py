...
while True:
    ...
    print("3. Salir")
    seleccion = int(input("[Seleccione del 1-3]: "))
    if seleccion == 1:
        cantidad_de_actividades = int(input("Ingrese la cantidad de actividades a registrar \n [Numero del 1 al 3]: "))
        for i in range(cantidad_de_actividades):
            actividad = ""
            actividad = input("Cual es el nombre de la actividad?: ")
            tiempo_total += int(input("Cuanto es el tiempo que dedica a esta actividad? (En minutos): "))
    if seleccion == 2:
        print(f"{tiempo_total} minutos")
        if tiempo_total > 180:
            print("Tiempo diario excesivo")
        if tiempo_total < 180:
            print("Tiempo diario adequado")
    if seleccion == 3:
