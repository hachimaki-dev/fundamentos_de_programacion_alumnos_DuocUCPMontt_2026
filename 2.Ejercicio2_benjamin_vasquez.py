...
while True:
    ...
    if opcion == "1":
        cantidad = 0
        while cantidad <= 3:
            cantidad = int(input("Ingrese la cantidad de actividades a registrar (mínimo 3): "))
        
        contador = 0
        acumulador = 0
        
        while contador < cantidad:
            nombre = input(f"Ingrese el nombre de la actividad {contador + 1}: ")
            tiempo = int(input(f"Ingrese el tiempo que toma '{nombre}' en minutos: "))
            acumulador += tiempo
            contador += 1
        
        total_tiempo += acumulador
        print(f"Se registraron {cantidad} actividades con un tiempo total de {acumulador} minutos.")
    
    elif opcion == "2":
       
        print(f"\nTiempo total acumulado: {total_tiempo} minutos")
        if total_tiempo > 180:
