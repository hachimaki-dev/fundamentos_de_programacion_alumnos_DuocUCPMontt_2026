...
def actualizar_precio(codigo, nuevo_precio):
    ...
    if codigo_upper in catalogo:
        catalogo[codigo_upper][0] = nuevo_precio
        return True
    return False

def agregar_serie(codigo,titulo,genero,estudio,clasificacion,subtitulado,pais_origen,precio,episodios):
    codigo_upper = codigo.strip().upper()
    series[codigo_upper] = [titulo,genero,estudio,clasificacion,subtitulado,pais_origen]
    catalogo[codigo_upper] = [precio,episodios]
    return True
    ...
    while True:
        ...
        if len(subtitulado)>0:
            if subtitulado in ["s","n"]:
                return True if subtitulado == "s" else False
            else:
...
def eliminar_serie(codigo):
    codigo_upper = codigo.strip().upper()
    if codigo_upper in catalogo:
        del series[codigo_upper]
        del catalogo[codigo_upper]
        return True
    return False
...
def iniciar_programa():
    while True:
        ...
        if opcion_elejida == "1":
                ...
                if len(codigo_usuario)>0:
                    try:
                        ...
                        if usuario_nuevo_precio >=0:
                            if actualizar_precio(codigo_usuario, usuario_nuevo_precio):
            ...
            if agregar_serie(codigo_usuario,titulo_usuario,genero_usuario,estudio_usuario,clasificacion_usuario,subtitulado_usuario,pais_usuario,precio_usuario,episodios_usuario):
            ...
            while True:
                ...
                if len(codigo_para_eliminar)>0:
                    if eliminar_serie(codigo_para_eliminar):
