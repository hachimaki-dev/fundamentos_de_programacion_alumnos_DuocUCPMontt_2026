...
def eliminar_bicho(nombre_de_bicho_a_eliminar):
    indice_del_bicho = buscar_nombre_del_bicho(nombre_de_bicho_a_eliminar)
    if lista_de_todos_los_bichos.pop(indice_del_bicho):
        return True
...
def iniciar_programa():
    while True:
        ...
        if opcion_menu_usuario_validada == "1":
            ...
            nombre_de_la_especie_a_buscar = input("Ingrese nombre del bicho a buscar \n[]: ")
            indice_encontrado = buscar_nombre_del_bicho(indice_de_bicho)

            if indice_encontrado is not None:
                print("Lo encontramos!")
                print(f"Nombre de la especie : {lista_de_todos_los_bichos[indice_encontrado]["especie_bicho"]}")
                print(f"Longitud de la especie : {lista_de_todos_los_bichos[indice_encontrado]["longitud_bicho"]}")
                print(f"Peligrosidad de la especie : {lista_de_todos_los_bichos[indice_encontrado]["peligrosidad_bicho"]}")
                print(f"Es peligroso? : {lista_de_todos_los_bichos[indice_encontrado]["es_peligroso"]}")
            else:
                print("No hay datos de esta clase")

        elif opcion_menu_usuario_validada == "3":
            nombre_del_bicho_a_eliminar = input("Ingrese el nombre del bicho a eliminar \n[]: ")
            se_elimino = eliminar_bicho(nombre_del_bicho_a_eliminar)
            if se_elimino is not None:
                print(f"{nombre_del_bicho_a_eliminar} existe!")
