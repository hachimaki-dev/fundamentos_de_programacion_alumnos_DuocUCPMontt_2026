...
# Restricciones: Recorre las cosas del piso con un `for`. 
# Agrégalas a tu mochila. Justo después de agregar, revisa si el largo (`len()`) de tu mochila es igual a 4. 
# Si es así, imprime 'Lleno' y corta el ciclo con `break`. 
...
for cosas in piso:
    if len(mochila) < 4:
        print(f"Has recogido {cosas}")
        mochila.append(cosas)
    else:
        print("Tu mochila está llena")
        break
