...
while opcion != 3:
    ...
    print("3. Salir")
    opcion = int(input("Porfavor selecciona una opcion: "))

    if opcion == 1:
        cantidad = int(input("¿Cual es la cantidad de actividades a registrar?(mayor o igual a 3): "))
        if cantidad >= 3:
            for i in range (cantidad):
                nombre = (input(f"¿Cual es el nombre de la actividad?: "))
                tiempo = int(input(f"¿Cuanto tiempo toma la actividad {nombre}?(en minutos): "))
                tiempo_total = tiempo_total + tiempo
                i = i + 1
        else:
            print("Ingrese un dato mayor o igual a 3 porfavor")
