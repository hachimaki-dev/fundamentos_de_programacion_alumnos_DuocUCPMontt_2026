...
while True:
    ...
    opciones=int(input("1)Registrar actividades \n2)Mostrar análisis del tiempo \n3)Salir \nSelccione una opcion: "))
    if opciones==1:
        actividades=int(input("Ingrese la cantidad de actividades a realizar: "))
        if actividades>=3:
            while contador<actividades:
                nombre_actividad= input("Ingrese el nombre de la actividad: ")
                tiempo_actividad= int(input("Ingrese el tiempo que le toma hacer esta actividad (en minutos): "))
                tiempo_total= tiempo_total+tiempo_actividad
                contador+=1
        else:
            print("Cantidad de actividades invalida.")
    elif opciones==2:
        print(f"Su tiempo total es de {tiempo_total} minutos")
        if tiempo_total>180:
