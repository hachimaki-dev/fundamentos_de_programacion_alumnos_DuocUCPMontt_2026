# 🎯 Reto 3: Armería de Valorant - Omitiendo objetos
# Tienes una tienda con 5 armas numeradas del 1 al 5. Vas a iterar sobre ellas, pero el arma 4 está "Agotada", así que debes saltarla.

# Instrucciones:

# Usa un while que cuente de 1 a 5 con una variable arma = 0 y vayas sumando.
# Dentro del ciclo, si arma == 4, imprime "Arma 4: Agotada" y usa un continue para saltar la compra y evaluar la siguiente.
# Para las demás armas, imprime "Comprando arma número X".
# Cuidado: Si usas continue, ¡asegúrate de haber incrementado tu variable antes, o tendrás ciclo infinito!

arma = 0

print("----- Armería de Valorant -----")

while arma < 5:
    arma += 1
    if arma == 4:
        print("Arma 4: Agotada")
        continue
    print(f"Comprando arma numero {arma}")